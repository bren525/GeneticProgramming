...
def plotting(results):
	...
	if ycheck[0][1] < ychecker[0][3]:
		ymin=ycheck[0][1]-margin
	else:
		ymin=ychecker[0][3]-margin
	if ycheck[len(ycheck)-1][1] > ychecker[len(ychecker)-1][3]:
		ymax=ycheck[len(ycheck)-1][1]+margin
	else:
		ymax=ychecker[len(ychecker)-1][3]+margin
	if xcheck[0][0] < xchecker[0][2]:
		xmin=xcheck[0][0] -margin
	else:
		xmin=xchecker[0][2]-margin
	if xcheck[len(xcheck)-1][0] > xchecker[len(xchecker)-1][2]:
		xmax=xcheck[len(xcheck)-1][0]+margin
	else:
		xmax=xchecker[len(xchecker)-1][2]+margin
